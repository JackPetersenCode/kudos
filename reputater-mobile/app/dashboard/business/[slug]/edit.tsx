...
import { ScrollView, View, Text, TextInput, Pressable, StyleSheet, ActivityIndicator, Alert, Switch, Platform } from "react-native";
import { Stack, useLocalSearchParams, router } from "expo-router";
import { useAuth } from "../../../../contexts/AuthContext";
import { getOwnedBusiness, updateBusiness, getBusinessHours, BusinessHourInput } from "../../../../lib/businessOwner";
...
export default function EditBusinessScreen() {
  ...
  const [form, setForm] = useState<Form>({
    name: "", description: "", phone: "", websiteUrl: "",
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        const data = await getOwnedBusiness(slug);
        let hours = DEFAULT_HOURS;
        try {
          const existing = await getBusinessHours(data.id);
          if (existing.length > 0) {
            hours = DEFAULT_HOURS.map((h) => {
              const e = existing.find((x) => x.dayOfWeek === h.dayOfWeek);
              return e ? { dayOfWeek: h.dayOfWeek, openTime: e.openTime ?? "09:00", closeTime: e.closeTime ?? "17:00", isClosed: e.isClosed } : h;
            });
          }
        } catch { /* ignore */ }
  ...
  const onSave = async () => {
    if (!slug) return;
    if (!form.name.trim()) {
      Alert.alert("Name required", "Please enter the business name.");
      return;
    }
    setSaving(true);
    try {
    ...
    <>
      ...
      <ScrollView style={{ flex: 1, backgroundColor: colors.bg }} contentContainerStyle={{ padding: 16, paddingBottom: 80 }}>
        ...
        <Section title="Category">
          ...
          <View style={styles.chipRow}>
            <Pressable
              onPress={() => updateField("categorySlug", form.categorySlug === "other" ? "" : "other")}
              style={[styles.chip, form.categorySlug === "other" && styles.chipActive]}
            >
              <Text style={[styles.chipText, form.categorySlug === "other" && styles.chipTextActive]}>Other</Text>
            </Pressable>
          </View>
        </Section>
